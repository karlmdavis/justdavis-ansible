...
1. WiFi associations: `{ap_mac: {band: {network_type: {client_mac: {Address?, HostName?, Mode?,
   ...
   or "Internal network" (mesh backhaul, which lacks Address/HostName/Mode).
2. Device directory: `{mac: {host_name?, description?, ip?, connection, icon_id, ...}}`.
3. Wired port map: `{router_mac: {client_mac: port}}` (unused).
...
def _mesh_point_name(friendly_name: str) -> str:
    """AmpliFi appends the model and serial to unnamed mesh points: "Living Room (AFi-P-HD-F656A8)"."""
...
def _parse_topology(element: object) -> tuple[Router, tuple[MeshPoint, ...]]:
    ...
    router = Router(
        mac=_mac(as_str(get(router_map, "mac", ctx), f"{ctx}.mac")),
    ...
    for mp_mac, mp_value in wifi_children.items():
        ...
        mesh_points.append(
            MeshPoint(
                mac=_mac(as_str(get(mp, "mac", mp_ctx), f"{mp_ctx}.mac")),
...
def _parse_directory(element: object) -> dict[str, DirectoryEntry]:
    directory: dict[str, DirectoryEntry] = {}
    for mac, entry_value in as_dict(element, "[2]").items():
        ctx = f"[2].{mac}"
        entry = as_dict(entry_value, ctx)
        description = _optional_str(entry, "description", ctx)
        host_name = _optional_str(entry, "host_name", ctx)
        directory[_mac(mac)] = DirectoryEntry(
            mac=_mac(mac),
            display_name=description or host_name or "",
            connection=as_str(get(entry, "connection", ctx), f"{ctx}.connection"),
            ip=_optional_str(entry, "ip", ctx),
        )
    return directory
...
def _parse_wan_port(element: object, router_mac: str) -> WanPort:
    ...
    for candidate_mac, candidate_value in ports_by_router.items():
        if _mac(candidate_mac) == router_mac:
...
def _parse_bonjour(element: object) -> dict[str, frozenset[str]]:
    ...
    for mac, entry_value in as_dict(element, "[5]").items():
        ...
        bonjour[_mac(mac)] = frozenset(service for service in types if service.startswith("_"))
    return bonjour
...
def parse_info_async(body: bytes | str) -> AmplifiSnapshot:
    ...
    return AmplifiSnapshot(
        ...
        directory=_parse_directory(elements[2]),
