...
class GatewaySettings:
    """The gateway is only ever spoken to over HTTPS with its certificate pinned; without a fingerprint
    the exporter idles rather than sending the admin credentials in cleartext."""
    ...
    def configured(self) -> bool:
        return self.host is not None and self.tls_fingerprint_sha256 is not None
    ...
    def from_env(cls, env: Mapping[str, str]) -> "GatewaySettings":
        ...
        return cls(
            ...
            username=username,
            password=password,
            tls_fingerprint_sha256=parse_fingerprint(
...
def build_registry() -> tuple[CollectorRegistry, GatewayCollector, Counter]:
    ...
    collector = GatewayCollector(snapshots, statuses)
...
def main() -> None:
    ...
    if settings.host is None:
    ...
    elif settings.tls_fingerprint_sha256 is None:
        log.error(
            "MONITORING_GATEWAY_TLS_FINGERPRINT_SHA256 is not set; refusing to talk to the gateway without "
        ...
        log.info("polling %s with certificate pinning", settings.base_url)
        ...
        thread = threading.Thread(
            ...
            kwargs={
                "scrape": GatewayScraper(client, collector, errors),
