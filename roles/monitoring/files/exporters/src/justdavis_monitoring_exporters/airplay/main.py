...
class AirplaySettings:
    ...
    def from_env(cls, env: Mapping[str, str]) -> "AirplaySettings":
        return cls(
            ...
            port=env_int(env, "MONITORING_AIRPLAY_PORT", 9803),
            listen_addr=env_str(env, "MONITORING_LISTEN_ADDR", "0.0.0.0"),
            interval_seconds=env_int(env, "MONITORING_AIRPLAY_INTERVAL", 30),
            resolve_timeout_ms=env_int(env, "MONITORING_AIRPLAY_RESOLVE_TIMEOUT_MS", 3000),
...
def main() -> None:
    ...
    shutdown = serve(AirplayCollector(snapshots, statuses), addr=settings.listen_addr, port=settings.port)
    ...
    if settings.lan_ip is None:
        ...
        def scrape() -> None:
            try:
                snapshot = poll(
                    active, settings.tracked_clients, timeout_ms=settings.resolve_timeout_ms, now=time.time()
                )
            except Exception:
                snapshots.clear()
                raise
